...
export async function POST(request) {
  const { name, email, message, phoneNumber, questionaire} =
    await request.json();
  ...
  const mailOptions = {
    from: email,
    to: "info@fmccng.com", // Replace with your receiving email
    subject: "FMCC",
    html: `
  ...
  try {
    ...
    console.error("❌ Nodemailer sendMail error:", error); // SHOW actual error
    return NextResponse.json(
